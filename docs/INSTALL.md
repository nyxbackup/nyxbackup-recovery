...
The RPM declares its dependencies as SONAMEs rather than package names (no
single RPM package-name set is correct across Fedora, RHEL, openSUSE and
Mageia), so it resolves on any of them: the WebKitGTK 4.1 / GTK 3 desktop
stack, plus `libsmbclient.so.0` for SMB shares.  All are present on a normal
desktop install; check with `rpm -qpR NyxBackup-Recovery-*.rpm`.
...
Leave **Mount path** blank on Linux and Windows.  Enter the share as
`smb://user@host/share/path`, put the share password in the SMB password
field, and the app connects on its own - Linux talks SMB directly through
libsmbclient, Windows connects to the UNC path.  Nothing has to be mounted,
and neither needs root or an administrator.

Set **Mount path** only if you already have the share mounted and want the app
to read through that mount instead.

macOS is the exception: it has no libsmbclient, so mount the share first and
set Mount path to the mount point.

```bash
mount_smbfs //user@host/share /Volumes/share    # then Mount path = /Volumes/share
```
