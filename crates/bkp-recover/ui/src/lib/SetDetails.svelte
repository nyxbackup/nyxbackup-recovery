...
<script lang="ts">
  ...
  function scheduleLine(r: SetRecordView): string {
    const time = (r.schedule_time || '').trim()
    const day = (r.schedule_day || '').trim()
    switch (r.schedule_kind) {
      // "manual" runs only on demand; the stored day/time are unused defaults
      // and must NOT be appended (that produced "manual monday 02:00").
      case 'manual':        return 'manual'
      case 'every_n_hours': return `every ${r.schedule_interval_hours}h`
      case 'daily':         return time ? `daily ${time}` : 'daily'
      case 'weekly':        return [day, time].filter(Boolean).length
                                     ? `weekly ${[day, time].filter(Boolean).join(' ')}`
                                     : 'weekly'
      case 'monthly':       return time ? `monthly ${time}` : 'monthly'
      default: {
        const extra = [day, time].filter(Boolean).join(' ')
...
{#if rec}
  <div class="border-b border-nyx-border bg-nyx-surface2/40 text-xs shrink-0">
    <button
      ...
      aria-expanded={open}
    >
      <!-- Name the set.  With several sets grouped in the left panel this
           header is otherwise anonymous, and the only clue about which set it
           describes is the highlighted row a column away. -->
      <span class="uppercase tracking-wider font-semibold">
        {t('gui.recover.browse.set_details.title')}{rec.set_name ? ` - ${rec.set_name}` : ''}
      </span>
