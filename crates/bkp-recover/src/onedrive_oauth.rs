...
// Compiled in from ONEDRIVE_OAUTH_CLIENT_ID when present (the official signed
// installers).  OneDrive uses the public-client flow (no secret); the default
// `common` tenant covers personal + work/school accounts.  `option_env!` (not
// `env!`) so a from-source build with no `.env` still COMPILES with an empty
// client id; the OneDrive backend then waits for a runtime override (BYO).
