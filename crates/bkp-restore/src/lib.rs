...
fn restore_checkpoints_dir() -> PathBuf {
    ...
    // Non-unsafe root detection.  The crate carries #![deny(unsafe_code)],
    // which a local #[allow] could override, so libc::getuid() is not actually
    // barred here - it is simply not worth an unsafe block.  (An earlier note
    // claimed #![forbid], which would have barred it; that was wrong, and the
    // same wrong claim was used elsewhere to defer real work.)  Daemon contexts
    // on Linux/macOS run with HOME=/root (Linux systemd default) or
    // HOME=/var/root (macOS launchd), and USER=root.  Either signal is enough:
    // we only choose between two well-known directories, and the file-store
    // fallback in bkp-daemon's keystore module uses the same trick on macOS.
    #[cfg(unix)]
    let is_root = std::env::var("USER").as_deref() == Ok("root")
        || std::env::var("HOME").as_deref() == Ok("/root")
        || std::env::var("HOME").as_deref() == Ok("/var/root");

    // No `return` here: the block below is cfg-stripped on macOS, so this is
    // already the function's tail expression - matching how that block is
    // written.
    #[cfg(target_os = "macos")]
    {
        if is_root {
...
/// Walk every pack object under `packs/` and build the
/// `chunk_id -> (pack_remote_path, offset, encrypted_size)` map the
...
///
/// Layout-agnostic (REQ-060): it accepts a pack object at any depth under
/// `packs/` whose trailing segment is `<uuid>.pack` - the flat legacy
/// `packs/<uuid>.pack`, the unified nested
/// `packs/<machine_id>/<set_id>/<uuid>.pack`, or a verbatim cross-backend copy
/// of either - and stores each pack's **actual discovered path** in the map, so
/// nothing downstream reconstructs a path from the pack id.
///
...
) -> bkp_types::error::Result<std::collections::HashMap<[u8; 32], (String, u64, u64)>> {
    let mut map = std::collections::HashMap::new();
    let raw_paths = storage.list("packs/").await?;
    let pack_paths: Vec<&String> = raw_paths
        .iter()
        .filter(|p| is_pack_object_path(p))
        .collect();

    for pack_path in &pack_paths {
        let pack_path = pack_path.as_str();
        let size = match storage.size(pack_path).await {
            Ok(s) => s,
            Err(e) => {
                tracing::warn!(pack = %pack_path, error = %e, "build_pack_map: size() failed; skipping pack");
                continue;
            }
        };
        if size < 30 {
        ...
        for e in &entries {
            map.insert(e.chunk_id, (pack_path.to_string(), e.offset, e.size));
        }
    }
    Ok(map)
}

/// True when `path`'s trailing segment is `<uuid>.pack` - a real pack object -
/// ignoring any namespace directories in front of it.  Layout-agnostic (REQ-060):
/// matches `packs/<uuid>.pack` and `packs/<machine_id>/<set_id>/<uuid>.pack`
/// alike, and rejects debris such as `<uuid>.pack.sav`.
fn is_pack_object_path(path: &str) -> bool {
    let name = path.rsplit('/').next().unwrap_or(path);
    match name.strip_suffix(".pack") {
        Some(stem) => uuid::Uuid::parse_str(stem).is_ok(),
        None => false,
    }
}
...
///
/// Returns `(pack_remote_path, byte_offset_of_size_prefix, encrypted_size)`.
/// The path is the actual object path as discovered from storage, so it is
/// correct regardless of pack layout - the flat legacy `packs/<uuid>.pack`, the
/// unified nested `packs/<machine_id>/<set_id>/<uuid>.pack` (REQ-060), or a
/// verbatim cross-backend copy of either.  No caller reconstructs a path from
/// the pack id.  Pass `None` for the `RestoreEngine` to fall back to individual
/// chunk objects.
pub type ChunkResolver = Arc<dyn Fn(&ChunkId) -> Option<(String, u64, u64)> + Send + Sync>;

/// One file collected from a manifest's file tree, as produced by
/// [`collect_files`]: `(relative_path, chunk_refs, mode_bits, symlink_target,
/// windows_attrs)`.  Named to keep the `Vec<...>` signatures under
/// `clippy::type_complexity`.
type CollectedFile = (PathBuf, Vec<ChunkRef>, u64, Option<String>, u32);
...
impl RestoreEngine {
    ...
    ///
    /// `pack_map` maps raw 32-byte chunk IDs to
    /// `(pack_remote_path, offset, encrypted_size)`.  Built by
    /// [`build_pack_map_from_storage`] (recovery tool) or from the local SQLite
    /// chunk index before spawning the restore task; the path is the actual
    /// discovered object path, so any pack layout resolves correctly.
    pub fn new_with_pack_cache(
        storage: Arc<dyn StorageBackend>,
        pack_map: HashMap<[u8; 32], (String, u64, u64)>,
    ) -> Self {
        let concurrency = storage.concurrency_hint().unwrap_or(8);
        let map = Arc::new(pack_map);
        let resolver: ChunkResolver = Arc::new(move |id: &ChunkId| map.get(id.as_bytes()).cloned());
        Self {
            storage,
            concurrency,
    ...
    pub async fn list_snapshots(
        ...
        // REQ-053: decode-aware critical read - falls back to the .bak mirror
        // when the primary is missing OR present-but-undecodable (self-heals).
        let index =
            bkp_storage::backend::get_critical_decoded(self.storage.as_ref(), &index_path, |d| {
                decode_snapshot_index(d, snapshot_index_key)
            })
            .await?;
    ...
    pub async fn restore_all(
        ...
        let mut raw_files: Vec<CollectedFile> = Vec::new();
        ...
        // Most-specific rule wins, matching the tree's `treeSelection.ts`.
        // The old code was two flat passes - any exclusion prefix vetoed the
        // path outright - so "restore this one file out of a folder I
        // deselected" was inexpressible: the tree painted the file ticked and
        // the restore dropped it anyway.
        if !filter_paths.is_empty() || !excluded_paths.is_empty() {
            raw_files.retain(|(p, _, _, _, _)| {
                let s = p.to_string_lossy().replace('\\', "/");
                path_selected(&s, filter_paths, excluded_paths)
            });
        }
        ...
        let packs_total_count: u64 = if let Some(ref resolve) = self.chunk_resolver {
            ...
            for t in tasks.iter() {
                ...
                for c in t.chunks.iter() {
                    if let Some((pack_path, _, _)) = resolve(&c.chunk_hash) {
                        uniq.insert(pack_path);
                    }
                }
            }
            uniq.len() as u64
    ...
    async fn fetch_manifest(
        ...
        // REQ-053: falls back to .bak on a missing OR undecodable primary.
        bkp_storage::backend::get_critical_decoded(self.storage.as_ref(), &path, |d| {
            decode_manifest(d, manifest_key)
        })
        .await
    }
}
...
fn classify_multi_chunk_packs(
    ...
    for file_chunks in pending {
        for cr in file_chunks {
            if let Some((pack_path, _, _)) = resolver(&cr.chunk_hash) {
                pack_chunks
                    .entry(pack_path)
                    .or_default()
    ...
    let write_result: Result<()> = async {
        for chunk_ref in &sorted_refs {
            ...
            let plaintext: Vec<u8> = if let Some(ref resolve) = resolver {
                if let Some((pack_path, offset, enc_size)) = resolve(&chunk_ref.chunk_hash) {
...
/// The `.par` sibling of a resolved pack path, if the path names a pack.
///
/// Derived from the path the pack was actually found at rather than rebuilt
/// from a prefix and an id: this tool accepts three pack layouts (flat legacy
/// `packs/<uuid>.pack`, unified nested `packs/<machine>/<set>/<uuid>.pack`, and
/// verbatim cross-machine copies), and swapping the extension is correct for
/// all three.
fn par_sibling_path(pack_path: &str) -> Option<String> {
    let name = pack_path.rsplit('/').next().unwrap_or(pack_path);
    let stem = name.strip_suffix(".pack")?;
    uuid::Uuid::parse_str(stem).ok()?;
    Some(format!("{}.par", pack_path.strip_suffix(".pack")?))
}

/// The [`bkp_types::snapshot::PackId`] named by a resolved pack path.
fn pack_id_from_path(pack_path: &str) -> Option<bkp_types::snapshot::PackId> {
    let name = pack_path.rsplit('/').next().unwrap_or(pack_path);
    let stem = name.strip_suffix(".pack")?;
    uuid::Uuid::parse_str(stem)
        .ok()
        .map(bkp_types::snapshot::PackId::from_uuid)
}

/// Recover a chunk whose ciphertext failed to decrypt, by reconstructing the
/// pack from its Reed-Solomon `.par` sibling.
///
/// A decrypt failure at this point means the stored pack bytes are damaged -
/// bit rot, a bad sector, a partial write.  This downloads the whole pack and
/// its parity, reconstructs, and decrypts the requested chunk from the repaired
/// bytes.
///
/// **The repaired pack is NOT written back.**  The main application heals the
/// stored object so later reads are clean; this tool deliberately does not.  It
/// is read-only by design and may be pointed at an archive the operator does
/// not own, and writing to someone's backup in the middle of their disaster
/// buys the restore in hand nothing.  The cost is that every damaged chunk is
/// repaired again on each restore, which is the right trade for a tool used
/// once.
async fn repair_and_decrypt_chunk(
    storage: &dyn StorageBackend,
    pack_path: &str,
    pack_offset: u64,
    encrypted_size: u64,
    ...
    let (Some(par_path), Some(pack_id)) =
        (par_sibling_path(pack_path), pack_id_from_path(pack_path))
    else {
        return Err(Error::Crypto(format!(
            "chunk in {pack_path} failed to decrypt, and that path does not name \
             a pack object so no parity sibling can be located"
        )));
    };
    let par_data = storage.get(&par_path).await.map_err(|_| {
        Error::Crypto(format!(
            "chunk in pack {pack_id} failed to decrypt and no parity (.par) is \
             available to repair it"
        ))
    })?;
    let pack_data = storage.get(pack_path).await?;
    let outcome = bkp_chunker::parity::repair_pack(pack_data, &par_data, &pack_id)?;
    if outcome.repaired {
        warn!(
            pack = %pack_id,
            erasures = outcome.erasures,
            "repaired a corrupt pack from its parity sibling (in memory; the \
             stored object is left untouched)"
        );
    }
    let from = (pack_offset + 4) as usize;
    let to = from + encrypted_size as usize;
    if to > outcome.bytes.len() {
        return Err(Error::Storage(format!(
            "chunk at offset {pack_offset} overruns repaired pack {pack_id}"
        )));
    }
    decrypt_and_decompress(
        &outcome.bytes[from..to],
        chunk_id,
        chunk_key_bytes,
        chunk_id_key_bytes,
    )
...
pub async fn fetch_chunk_ranged(
    ...
    match decrypt_and_decompress(&encrypted, chunk_id, chunk_key_bytes, chunk_id_key_bytes) {
        Ok(plain) => Ok(plain),
        Err(first) => {
            // Damaged bytes are the likely cause, so try the parity sibling
            // before giving up.  If there is no `.par`, or the damage exceeds
            // what it can reconstruct, report the ORIGINAL decrypt failure -
            // "no parity available" would describe our recovery attempt rather
            // than what is actually wrong with the archive.
            match repair_and_decrypt_chunk(
                storage,
                pack_path,
                pack_offset,
                encrypted_size,
...
async fn fetch_chunk(
    ...
    if let Some(resolve) = resolver
        && let Some((pack_path, offset, enc_size)) = resolve(chunk_id)
    {
...
/// Drop zero-length chunk references from a file's chunk list.
///
/// A real content chunk is always at least one byte, so a `plaintext_size == 0`
/// `ChunkRef` never carries data.  Such refs only ever arose from the empty-file
/// phantom-chunk bug in the main app's engine: an unchanged 0-byte file used to
/// be recorded with a bogus single chunk pointing at a hash that was never
/// stored.  Old snapshots still contain that phantom, so the Recovery Tool must
/// ignore it - otherwise a set that holds an empty file restores that file as
/// "skipped, pack missing" instead of as the empty file it is.
fn content_chunks(chunks: &[ChunkRef]) -> Vec<ChunkRef> {
    chunks
        .iter()
        .filter(|c| c.plaintext_size > 0)
        .cloned()
        .collect()
}

fn collect_files(node: &TreeNode, prefix: &Path, out: &mut Vec<CollectedFile>) {
...
/// Normalise a selection / exclusion rule for comparison: forward slashes,
/// no leading separator.
fn norm_rule(p: &str) -> String {
    p.replace('\\', "/").trim_start_matches('/').to_string()
}

/// True when `path` is `rule` itself or lies beneath it.
fn at_or_under(path: &str, rule: &str) -> bool {
    path == rule || path.starts_with(&format!("{rule}/"))
}

/// Decide whether one file path survives a partial restore's filters.
///
/// `filter_paths` (selections) and `excluded_paths` are both *prefix rules*,
/// and the MOST SPECIFIC rule covering `path` decides.  That is what lets a
/// selection nested inside an excluded folder be restored, and an exclusion
/// nested inside a selected folder be skipped.
///
/// The previous rule was flat: any exclusion prefix vetoed the path outright,
/// so "restore this one file out of a folder I excluded" was inexpressible.
/// The GUI could paint the file selected and the restore would still drop it.
///
/// On a tie (the same path appears in both lists) the exclusion wins, since
/// not writing a file is the recoverable mistake.  With no rule covering
/// `path` at all, an empty `filter_paths` means "restore everything".
pub fn path_selected(path: &str, filter_paths: &[String], excluded_paths: &[String]) -> bool {
    let path = norm_rule(path);
    let mut best: Option<(usize, bool)> = None;
    for (rules, include) in [(filter_paths, true), (excluded_paths, false)] {
        for r in rules {
            let r = norm_rule(r);
            if !at_or_under(&path, &r) {
                continue;
            }
            // `>=` on the exclusion pass is the tie-break: equal specificity
            // resolves to excluded.
            let wins = match best {
                None => true,
                Some((len, _)) if include => r.len() > len,
                Some((len, _)) => r.len() >= len,
            };
            if wins {
                best = Some((r.len(), include));
            }
        }
    }
    match best {
        Some((_, include)) => include,
        None => filter_paths.is_empty(),
    }
}

/// Decide whether a directory should be pre-created for a partial restore.
///
/// Same longest-prefix rule as [`path_selected`], plus one extra: a directory
/// is kept when any selection lies strictly beneath it, even if the directory
/// itself is excluded.  Without that, selecting one file inside an excluded
/// folder would have nowhere to land.
pub fn dir_selected(path: &str, filter_paths: &[String], excluded_paths: &[String]) -> bool {
    let p = norm_rule(path);
    if filter_paths
        .iter()
        .any(|fp| norm_rule(fp).starts_with(&format!("{p}/")))
    {
        return true;
    }
    path_selected(&p, filter_paths, excluded_paths)
}
...
mod tests {
    ...
    #[test]
    fn path_selected_longest_prefix_wins() {
        let sel = vec!["Claudine".to_string()];
        let exc = vec!["Claudine/school".to_string()];

        assert!(path_selected("Claudine/notes.txt", &sel, &exc));
        assert!(!path_selected("Claudine/school/essay.txt", &sel, &exc));

        // The case this change exists for: one file picked out of an excluded
        // folder.
        let sel2 = vec![
            "Claudine".to_string(),
            "Claudine/school/essay.txt".to_string(),
        ];
        assert!(path_selected("Claudine/school/essay.txt", &sel2, &exc));
        assert!(!path_selected("Claudine/school/other.txt", &sel2, &exc));
    }

    #[test]
    fn path_selected_handles_ties_separators_and_empty_filters() {
        // Same path in both lists: exclusion wins.
        let both = vec!["a/b".to_string()];
        assert!(!path_selected("a/b", &both, &both));

        // Backslashes and leading separators normalise.
        let sel = vec!["\\Claudine\\school".to_string()];
        assert!(path_selected("/Claudine/school/x.txt", &sel, &[]));

        // A sibling sharing a name prefix is not covered by the rule.
        let sel = vec!["a/bc".to_string()];
        assert!(!path_selected("a/bcd.txt", &sel, &[]));

        // No filters at all means restore everything; an exclusion alone
        // still applies.
        assert!(path_selected("anything", &[], &[]));
        assert!(!path_selected("a/b/c", &[], &["a/b".to_string()]));
        assert!(path_selected("a/z", &[], &["a/b".to_string()]));
    }
}
