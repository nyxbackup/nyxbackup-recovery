...
// Compiled in from GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET when
// present (the official signed installers).  `option_env!` (not `env!`) so a
// from-source build with no `.env` still COMPILES with empty creds; OAuth
// backends then wait for a runtime override (BYO).
