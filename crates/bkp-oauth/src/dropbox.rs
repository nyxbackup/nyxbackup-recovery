...
///
/// The Recovery Tool is restore-only, so it requests a **read-only** subset of
/// the shared app's scopes (`account_info.read`, `files.metadata.read`,
/// `files.content.read`).  Dropbox honors a per-authorize scope subset even
/// though the app registration also holds write scopes, so the token this flow
/// mints can list and download the backup, but provably cannot write to or
/// delete the user's Dropbox.
fn build_auth_url(creds: &DropboxCreds, redirect_uri: &str) -> String {
    format!(
        ...
        scope = urlencode("account_info.read files.metadata.read files.content.read"),
    )
}
