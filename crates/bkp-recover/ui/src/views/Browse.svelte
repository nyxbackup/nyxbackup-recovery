...
<script lang="ts">
  ...
  import SetDetails from '../lib/SetDetails.svelte'
  ...
  // Backup sets present at this endpoint that the in-session master key could
  // not open.  Shown as a non-fatal notice: without it, a set from another
  // machine is invisible and reads as "my backup is gone".
  let undecryptableSets = $state(0)
  ...
  // Group snapshots by backup set.  A backup set and a snapshot are distinct
  // concepts - a set can hold many snapshots - so the panel renders a set
  // header with its dated snapshots nested underneath, never a flat list that
  // conflates the two.  Sets keep first-appearance order; snapshots newest first.
  const groupedSnapshots = $derived.by(() => {
    const groups: { set_id: string; label: string; snaps: SnapshotSummary[] }[] = []
    const byId: Record<string, { set_id: string; label: string; snaps: SnapshotSummary[] }> = {}
    for (const s of snapshots) {
      let g = byId[s.set_id]
      if (!g) { g = { set_id: s.set_id, label: setLabel(s), snaps: [] }; byId[s.set_id] = g; groups.push(g) }
      g.snaps.push(s)
    }
    for (const g of groups) {
      g.snaps.sort((a, b) => (a.created_at < b.created_at ? 1 : a.created_at > b.created_at ? -1 : 0))
    }
    return groups
  })
  ...
  async function loadSnapshots() {
    ...
    snapsError = ''
    try {
      const reply = await api.listSnapshots()
      snapshots = reply.snapshots
      undecryptableSets = reply.undecryptable_sets
...
<div class="grid grid-cols-[260px_1fr_320px] gap-3 p-4 h-full">
  ...
  <div class="flex flex-col bg-nyx-surface border border-nyx-border rounded-lg overflow-hidden">
    ...
    <div class="flex-1 overflow-auto">
      ...
      {:else}
        {#if undecryptableSets > 0}
          <!-- Present-but-unopenable sets.  Never an error (we did open at
               least one), but the user must not read the short list as data
               loss.  Names the likely cause and the fix. -->
          <div class="px-3 py-2 text-[11px] leading-snug text-nyx-warning bg-nyx-warning/10 border-b border-nyx-border">
            {tf('gui.recover.browse.undecryptable_sets', { count: undecryptableSets })}
          </div>
        {/if}
        {#each groupedSnapshots as group (group.set_id)}
          <div class="px-3 py-1.5 bg-nyx-surface2/60 border-b border-nyx-border">
            <div class="text-[11px] font-semibold text-nyx-text truncate" title={group.label}>{group.label}</div>
          </div>
          {#each group.snaps as snap (snap.snapshot_id)}
            <button
              onclick={() => pickSnapshot(snap)}
              disabled={selectionLocked}
              class="w-full text-left pl-5 pr-3 py-2 text-xs border-b border-nyx-border/50 disabled:opacity-40 disabled:cursor-not-allowed
                     {selectedSnap?.snapshot_id === snap.snapshot_id
                       ? 'bg-nyx-accent/20 text-nyx-text'
                       : 'text-nyx-muted hover:bg-nyx-surface2'}"
            >
              <div class="font-medium truncate">{formatDate(snap.created_at)}</div>
  ...
  <!-- ── Centre: File tree ────────────────────────────────────────────── -->
  <div class="flex flex-col min-h-0 bg-nyx-surface border border-nyx-border rounded-lg overflow-hidden
    ...
    <SetDetails setId={selectedSnap?.set_id} />
