...
//!
//! Scopes:
//!   - `Files.Read` - **read-only** access to the user's OneDrive files.  The
//!     Recovery Tool only lists and downloads the backup; it never writes or
//!     deletes, so the write scope (`Files.ReadWrite`, used by the main app)
//!     is deliberately dropped here.  Microsoft's v2 endpoint grants this
//!     narrower scope dynamically - no app-registration change needed.
...
/// Bundled OAuth client credentials.  Pass
/// `option_env!("ONEDRIVE_OAUTH_CLIENT_ID")` from the caller so the value
/// stays embedded in each individual binary, not in this shared crate.
///
/// There is deliberately no client secret: this is a public-client flow
/// (see the token exchange below), and Microsoft rejects a secret with
/// AADSTS90023.  The loopback redirect is what secures the exchange.
...
///
/// The Recovery Tool is restore-only, so it requests **`Files.Read`** (not
/// `Files.ReadWrite`) - it can list and download the backup but provably cannot
/// write to or delete the user's OneDrive.  Microsoft's v2 endpoint allows
/// this narrower scope dynamically; no app-registration change is needed.
const SCOPE: &str = "Files.Read offline_access User.Read openid email";
