...
# would turn WORKSPACE_DIR-derived paths absolute (/target/..., /dist/...).
# Fail loudly instead.
[[ -n "$WORKSPACE_DIR" && -d "$WORKSPACE_DIR" ]] || {
    echo "ERROR: could not resolve WORKSPACE_DIR from the script path." >&2
    exit 1
}
# Load .env so the signing gate below sees NYX_SIGN_KEY_ID.  The chained deb
# script sources .env too, but that runs as a subprocess - its exports do not
# reach back here, and the rpmsign step runs in THIS script's env.  Without its
# own load the gate silently takes the "unsigned (dev build)" branch and still
# exits 0, so a release build looks clean while shipping an unsigned .rpm.
...
if [[ -n "${NYX_SIGN_KEY_ID:-}" ]]; then
    echo "NYX_SIGN_KEY_ID set - signing .rpm with rpmsign..."
    bash "${WORKSPACE_DIR}/scripts/linux/sign_rpm.sh" "$OUTPUT" "$NYX_SIGN_KEY_ID"
    echo ""
else
    echo "NYX_SIGN_KEY_ID not set - .rpm is unsigned (dev build)."
    echo ""
fi
