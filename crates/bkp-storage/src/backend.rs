...
/// Read a critical small object at `path`, **decode it**, and transparently fall
/// back to the `<path>.bak` mirror when the primary is missing OR present but
/// undecodable (bit rot, a torn/partial upload, a half-completed overwrite).
///
/// The decode-aware companion to [`StorageBackend::get_critical`], which only
/// reaches for the mirror when the primary *download* fails - a primary that
/// downloads fine but fails AEAD/CBOR decode never triggers the fallback, so
/// the redundant `.bak` beside it goes unused exactly when it is needed
/// (REQ-053).  This runs the caller's `decode` closure and treats a decode
/// failure the same as a missing primary.  Unlike the main app there is no
/// self-heal step - the Recovery Tool is strictly read-only (the trait has no
/// `put`), so it decodes the good mirror and returns without rewriting.
///
/// Error semantics preserved for callers that special-case not-found: both
/// copies genuinely absent -> a `StorageMissingObject`-classified error; a copy
/// exists but neither decodes -> the decode error (never not-found).
///
/// A free function rather than a trait method because `StorageBackend` is used
/// as `dyn StorageBackend`; a generic method would break object safety.
pub async fn get_critical_decoded<T>(
    storage: &dyn StorageBackend,
    path: &str,
    decode: impl Fn(&[u8]) -> Result<T>,
) -> Result<T> {
    use bkp_types::error::{ErrorCategory, classify_error};

    // --- primary: download, then decode ---
    let mut primary_get_err: Option<Error> = None;
    let mut primary_decode_err: Option<Error> = None;
    match storage.get(path).await {
        Ok(bytes) => match decode(&bytes) {
            Ok(value) => return Ok(value),
            Err(e) => primary_decode_err = Some(e),
        },
        Err(e) => primary_get_err = Some(e),
    }

    // Primary unusable (missing or corrupt).  Try the mirror.
    let bak_path = format!("{path}.bak");
    let bak_err: Option<Error> = match storage.get(&bak_path).await {
        Ok(bytes) => match decode(&bytes) {
            Ok(value) => {
                let why = primary_decode_err
                    .as_ref()
                    .map(|e| e.to_string())
                    .or_else(|| primary_get_err.as_ref().map(|e| e.to_string()))
                    .unwrap_or_default();
                tracing::warn!(target: "bkp_storage",
                    "primary critical object {path} unusable ({why}); recovered by \
                     decoding .bak (read-only: primary not rewritten)");
                return Ok(value);
            }
            Err(e) => Some(e),
        },
    ...
    // Both copies failed.  Preserve not-found ONLY when BOTH are genuinely
    // absent - otherwise some data exists but nothing decoded (corruption).
    let primary_absent = primary_get_err
        .as_ref()
        .map(|e| classify_error(e) == ErrorCategory::StorageMissingObject)
        .unwrap_or(false);
    let bak_absent = bak_err
        .as_ref()
        .map(|e| classify_error(e) == ErrorCategory::StorageMissingObject)
        .unwrap_or(false);

    if primary_absent && bak_absent {
        return Err(primary_get_err.expect("primary_absent implies Some"));
    }

    let primary_why = primary_decode_err
        .as_ref()
        .map(|e| e.to_string())
        .or_else(|| primary_get_err.as_ref().map(|e| e.to_string()))
        .unwrap_or_default();
    tracing::error!(target: "bkp_storage",
        "CRITICAL: primary and .bak both unusable for {path}: primary={primary_why}; bak={}",
        bak_err.as_ref().map(|e| e.to_string()).unwrap_or_default());
    Err(primary_decode_err
        .or(bak_err)
        .or(primary_get_err)
