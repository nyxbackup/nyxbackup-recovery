...
<script lang="ts">
  ...
  function openSite() { openExternal('https://nyxbackup.com/download-recovery') }
  function openSource() { openExternal('https://github.com/nyxbackup/nyxbackup-recovery') }
  // Published on-disk format spec + reference test vectors (REQ-066): the
  // "open format" leg of the standalone-binary / open-source / open-format
  // trust story.  Anyone can build an independent reader from it.
  function openFormat() { openExternal('https://nyxbackup.com/format') }
</script>
...
<div class="max-w-xl mx-auto p-6 flex flex-col gap-4">
  ...
  <section class="rounded-lg border border-nyx-border bg-nyx-surface p-4 flex flex-col gap-2">
    ...
    >https://nyxbackup.com/download-recovery</button>
    ...
    <button
      onclick={openFormat}
      class="self-start text-xs underline text-nyx-accent hover:text-nyx-accent-hi"
    >https://nyxbackup.com/format</button>
