...
impl StorageBackend for OneDriveBackend {
    ...
    async fn head_with_hash(&self, path: &str) -> Result<(u64, String, String)> {
        // `id` and `name` are required fields on `DriveItem`; Graph only
        // returns a field when it is named in `$select`, so omitting them
        // makes deserialization fail with "missing field `id`".  Select
        // every field the struct needs, matching the listing query above.
        let url = format!("{}?$select=id,name,size,file", self.item_url(path));
        self.with_token(|token| {
            let url = url.clone();
            let path = path.to_string();
            async move {
