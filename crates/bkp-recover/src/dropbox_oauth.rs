...
// Compiled in from DROPBOX_APP_KEY / DROPBOX_APP_SECRET when present in the
// build environment (the official signed installers).  `option_env!` (not
// `env!`) so a from-source build with no `.env` still COMPILES - it just ships
// empty creds, and the OAuth backends wait for a runtime override (BYO) while
// every non-OAuth backend (S3/B2/SFTP/local/...) works immediately.
