...
impl GoogleDriveBackend {
    ...
    /// True when this backend targets the hidden appDataFolder space
    /// (drive.appdata). Files there live in their own Drive "space", so every
    /// query must name it explicitly with `spaces=appDataFolder`, else Drive
    /// searches the regular My Drive space and finds nothing.
    fn is_appdata(&self) -> bool {
        self.folder_id == "appDataFolder"
    }
    ...
    async fn find_file(&self, name: &str, token: &str) -> Result<Option<DriveFile>> {
        ...
        if self.is_appdata() {
            params.push(("spaces", "appDataFolder"));
        }
        let resp = self
            .client
