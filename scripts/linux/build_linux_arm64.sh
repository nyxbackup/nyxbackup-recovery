...
#
# GLIBC / BUILD HOST: cross-compiling does NOT insulate this from the host's
# glibc.  The aarch64 sysroot is assembled from the host's arm64 packages, so a
# build on Ubuntu 24.04 produces a GLIBC_2.39 binary and the packaging step
# refuses it against its 2.35 ceiling:
#
#     ERROR: binary requires GLIBC_2.39, ceiling is GLIBC_2.35.
#
# Build on Ubuntu 22.04 (glibc 2.35), exactly as build_linux_x86_64.sh
# documents for its own architecture.  Both Linux architectures need that host;
# only the Windows builds are unaffected.  Signing is separate and happens
# wherever the GPG key lives - it does not have to be the same machine.
#
...
fetch_arm64_smbclient() {
    ...
    (
        ...
        local p
        for p in "${pkgs[@]}"; do
            # Two separate Ubuntu renames have to be tolerated here:
            #   24.04 renamed the runtime package libsmbclient -> libsmbclient0
            #   the 64-bit time_t transition appended t64 to a swathe of
            #   library packages, e.g. libtevent0 -> libtevent0t64
            # Plain name first, so hosts that still carry the un-suffixed
            # package (older Ubuntu, Debian) are unaffected.
            apt-get download "${p}:arm64" 2>/dev/null \
                || apt-get download "${p}0:arm64" 2>/dev/null \
                || apt-get download "${p}t64:arm64" 2>/dev/null \
                || { echo "ERROR: could not download ${p}:arm64 (tried also ${p}0, ${p}t64)"; exit 1; }
        done
    ) || { rm -rf "$tmp"; return 1; }
...
python3 "${WORKSPACE_DIR}/scripts/common/check_runtime_deps.py" linux arm64 "$STAGING"
