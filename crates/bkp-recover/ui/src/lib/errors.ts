 ...
 *
 * An optional FOURTH field carries an actionable shell command (currently only
 * the "SMB share is not mounted" case, where recovery cannot mount the share
 * itself and the user has to run the command).  Unlike the detail it IS shown,
 * appended to the translated sentence; it is a literal command line, so it is
