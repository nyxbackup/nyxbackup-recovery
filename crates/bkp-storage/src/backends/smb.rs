...
impl StorageBackend for SmbBackend {
    ...
    async fn probe_access(&self) -> Result<()> {
        ...
        // Mount-path fallback.  `Path::is_dir` is a synchronous stat and an
        // `async fn` body runs on a tokio WORKER thread, so against a hung or
        // unreachable mount this held a worker until the OS gave up.  The
        // libsmbclient path above is already offloaded; this one was not.
        let root = self.full_path("");
        let probe = root.clone();
        let is_dir = tokio::task::spawn_blocking(move || probe.is_dir())
            .await
            .map_err(|e| Error::Internal(format!("SMB probe join: {e}")))?;
        if is_dir {
            return Ok(());
        }
    ...
    async fn exists(&self, path: &str) -> Result<bool> {
        ...
        // Same reasoning as probe_access: a synchronous stat on the mount path
        // must not run on a worker thread.
        let full = self.full_path(path);
        tokio::task::spawn_blocking(move || full.exists())
            .await
            .map_err(|e| Error::Internal(format!("SMB exists join: {e}")))
    }
