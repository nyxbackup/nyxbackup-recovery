...
sets/<backup_set_id>/set-record                     # encrypted envelope (optional)
...
Packs are namespaced by machine and set
(`packs/<machine_id>/<backup_set_id>/`, REQ-060) so multiple machines - or
multiple sets - can share one destination safely; each set owns its own pack
subtree.  Older backups and verbatim cross-backend copies may still use the
**flat** `packs/<pack_id>.pack` layout.  A reader MUST therefore locate packs
**layout-agnostically**: list `packs/` and treat any object whose trailing
segment is `<pack_id>.pack` as a pack, at any depth, using its actual path.
Pack UUIDs are random v4 and globally unique, so the machine/set directories
are not needed to identify a pack - the reader never reconstructs a path from
the pack id.  (The `packs/` prefix is preserved so object-store storage-class
tiering still recognises pack objects.)

The optional `sets/<backup_set_id>/set-record` object is an encrypted,
descriptive per-set metadata blob (name, owning machine/user, source paths,
retention, schedule).  It exists only as a recovery/management convenience for
naming sets; restore never depends on it and a reader may ignore it.
...
Pack files (at `packs/<machine_id>/<backup_set_id>/<pack_id>.pack`, or
flat `packs/<pack_id>.pack` in older/verbatim-copied backups) carry the
actual encrypted chunk bytes that reconstruct file contents.  Pack files
are NOT wrapped in a BKEV envelope - they have their own format.
...
First resolve each pack id to its actual object path.  List `packs/`
and treat any object whose trailing segment is `<pack_id>.pack` as a
pack (layout-agnostic: matches both `packs/<pack_id>.pack` and
`packs/<machine_id>/<backup_set_id>/<pack_id>.pack`).  Key the map by
the discovered **path**, never a reconstructed one.

```
pack_paths: map[pack_id -> object_path] = { trailing_uuid(p) -> p
                                            for p in list("packs/")
                                            if p ends_with ".pack" }
pack_index: map[chunk_id -> (pack_path, offset_in_pack, encrypted_size)] = empty

for pack_id in entry.packs_referenced:
    pack_path = pack_paths[pack_id]
    pack_bytes = dest_objects[pack_path]
    ...
    for e in entries:
        pack_index[e.chunk_id] = (pack_path, e.offset, e.size)
```
...
for chunk_ref in this_node.file_entry.chunks:
    # Locate the encrypted chunk in its pack (path from the resolver above).
    (pack_path, offset, enc_size) = pack_index[chunk_ref.chunk_hash]
    pack_bytes = dest_objects[pack_path]
