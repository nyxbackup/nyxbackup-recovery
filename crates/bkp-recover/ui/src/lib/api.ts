...
/// Reply from `rec_list_snapshots`.  `undecryptable_sets` counts backup sets
/// present at the endpoint that this master key cannot open - normally 0, but
/// non-zero when one endpoint holds sets from several machines (each machine
/// has its own master key).  The UI must surface it: otherwise a set that is
/// present but unopenable is indistinguishable from a set that is gone.
export interface SnapshotListReply {
  snapshots: SnapshotSummary[]
  undecryptable_sets: number
}
...
export const api = {
  ...
  listSnapshots:  (): Promise<SnapshotListReply> => invoke('rec_list_snapshots'),
