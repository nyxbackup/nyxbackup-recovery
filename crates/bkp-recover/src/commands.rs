...
use crate::session::{
    EndpointParams, Phase, RestoreProgressView, SharedSession, SnapshotListReply, SnapshotSummary,
};
use crate::settings::Settings;
use bkp_crypto::keys::MasterKey;
use bkp_manifest::{
    decode_manifest, decode_set_record, decode_snapshot_index, manifest_remote_path,
    set_record_remote_path, snapshot_index_remote_path,
};
...
pub async fn rec_test_connection(args: ConnectArgs) -> Result<(), String> {
    ...
    // Probe reachability BEFORE listing.  On the filesystem-backed backends
    // (SMB, Local) `list()` returns an empty Vec when the storage root does
    // not exist - the directory walk treats an unreadable directory as "no
    // entries".  An unmounted SMB share would therefore report a SUCCESSFUL
    // connection that simply contains no snapshots.  `probe_access` is the
    // explicit "is the configured root actually an accessible directory"
    // check, and every backend implements it.
    let probe = async {
        backend.probe_access().await?;
        backend.list("manifests/").await.map(|_| ())
    };
    match tokio::time::timeout(TEST_TIMEOUT, probe).await {
        Ok(Ok(())) => {}
        Ok(Err(e)) => {
...
pub async fn rec_connect(
    ...
    // See the note in rec_test_connection: `list()` cannot distinguish "empty"
    // from "root missing" on the filesystem-backed backends, so check access
    // explicitly first.  Without this an unmounted SMB share connects
    // "successfully" and then shows zero snapshots.
    tracing::info!("rec_connect: backend built, probing access...");
    if let Err(e) = backend.probe_access().await {
        tracing::warn!(error = %e, "rec_connect: probe_access failed");
        return Err(ue(Ctx::Connect, &e));
    }

    tracing::info!("rec_connect: access ok, probing manifests/...");
...
pub async fn rec_list_snapshots(
    session: State<'_, SharedSession>,
) -> Result<SnapshotListReply, String> {
    ...
    let reply = discover_snapshots(backend.as_ref(), &master_key).await?;

    // Cache for downstream lookups.
    {
        let mut s = session.write().await;
        s.snapshots = reply.snapshots.clone();
    }

    Ok(reply)
}

/// Discover every backup set at `backend` and load the snapshots of those the
/// `master_key` can open.
///
/// Split out of [`rec_list_snapshots`] so it can be tested without Tauri
/// session state: the interesting case is a MIXED-KEY endpoint, where some
/// sets decode and others do not.  Master keys are per-machine and some
/// endpoints hold several machines' sets side by side (Google Drive's
/// `appDataFolder` is one flat per-account space), so "present but
/// unopenable" is a normal state, not a corruption.
///
/// Returns the snapshots plus a count of the sets that could not be opened.
/// Callers MUST surface that count - a silently short list is
/// indistinguishable from lost data to a user who is already having a bad day.
///
/// Errors only when NOTHING decodes, which means the key is wrong for this
/// endpoint entirely rather than merely wrong for some of its sets.
pub async fn discover_snapshots(
    ...
    for set_id in &set_ids {
        ...
        // when the primary is missing OR present-but-undecodable, so a corrupt
        // primary snapshot-index no longer hides a set that has a good mirror.
        let index = match bkp_storage::backend::get_critical_decoded(backend, &path, |d| {
            decode_snapshot_index(d, &key)
        })
        .await
        {
            Ok(i) => i,
            Err(e) => {
                // Reached only after BOTH primary and .bak failed.  Distinguish
                // "envelope can't even be parsed" (legacy cipher_id != 0, magic
                // mismatch, body length wrong) from "envelope parses but auth
                // tag fails" (wrong master key) from a plain fetch failure.  A
                // legacy-cipher index needs a different fix (rebuild the remote
                // snapshot-index by running a backup on the source machine); a
                // key mismatch is a key problem.  The SAME master key can appear
                // to work elsewhere while the snapshot-index on B2 is still in
                // cipher_id=2 legacy format: a local DB cache can hide it, but
                // recovery has no cache and reads the encrypted object directly.
                let msg = format!("{e}");
                ...
                tracing::warn!(
                    ...
                    "fetch/decode snapshot-index failed"
                );
                decode_failures += 1;
                continue;
            }
        ...
        if let Some(latest) = index.entries.last() {
            ...
            if let Some(mk) = manifest_subkey {
                ...
                match bkp_storage::backend::get_critical_decoded(backend, &mpath, |d| {
                    decode_manifest(d, &mk)
                })
                .await
                {
                    Ok(m) => {
                    ...
                    Err(e) => tracing::warn!(set_id = %set_id.as_uuid(),
                        error = %e, "fetch/decode manifest for label failed"),
                }
            }
        }
    ...
    // Report the sets we could see but not open.  Reaching here means at
    // least one set DID decode (the all-failed case returned above), so any
    // remaining failures are the mixed-key endpoint: the user is looking at a
    // partial list and has no other way to know it.
    Ok(SnapshotListReply {
        snapshots,
        undecryptable_sets: decode_failures,
    })
}

/// Descriptive per-set metadata surfaced from a set-record for the details
/// panel.  All fields come from `sets/<set_id>/set-record` (encrypted).
#[derive(Serialize)]
pub struct SetRecordView {
    pub set_id: String,
    ...
    pub retention_keep_all_days: u32,
    pub retention_keep_weekly_count: u32,
    pub retention_keep_monthly_count: u32,
...
/// Read a backup set's optional descriptive set-record (REQ-060).
///
/// Returns `Ok(None)` when no set-record exists (older sets, or sets that have
/// not run since the feature shipped) or it cannot be decoded - the panel then
/// simply shows nothing extra.  Purely informational; never required for
/// restore.
#[tauri::command]
pub async fn rec_get_set_record(
    session: State<'_, SharedSession>,
    set_id: String,
) -> Result<Option<SetRecordView>, String> {
    let uuid =
        Uuid::parse_str(&set_id).map_err(|_| user_error(Ctx::Generic, "invalid backup set id"))?;
    let sid = BackupSetId::from_uuid(uuid);
    ...
    let key = match bkp_crypto::subkey::derive_subkey(
        ...
        Ok(k) => k,
        Err(e) => {
            tracing::warn!(set_id = %sid.as_uuid(), error = %e, "rec_get_set_record: derive subkey failed");
            return Ok(None);
        }
    };

    // The set-record is a single object with no `.bak` mirror; a plain get is
    // correct.  Absence is expected and not an error.
    let path = set_record_remote_path(&sid);
    let data = match backend.get(&path).await {
        Ok(d) => d,
        Err(e) => {
            tracing::info!(set_id = %sid.as_uuid(), error = %e, "rec_get_set_record: no set-record present");
            return Ok(None);
        }
    };

    let rec = match decode_set_record(&data, &key) {
        Ok(r) => r,
        Err(e) => {
            tracing::warn!(set_id = %sid.as_uuid(), error = %e, "rec_get_set_record: decode failed");
            return Ok(None);
        }
    };

    Ok(Some(SetRecordView {
        set_id: rec.backup_set_id.as_uuid().to_string(),
        machine_id: rec.machine_id.as_uuid().to_string(),
        set_name: rec.set_name,
        hostname: rec.hostname,
        username: rec.username,
        enabled: rec.enabled,
        include_paths: rec.include_paths,
        exclude_paths: rec.exclude_paths,
        exclude_patterns: rec.exclude_patterns,
        exclude_extensions: rec.exclude_extensions,
        endpoint_type: rec.endpoint_type,
        endpoint_target: rec.endpoint_target,
        retention_keep_all_days: rec.retention_keep_all_days,
        retention_keep_weekly_count: rec.retention_keep_weekly_count,
        retention_keep_monthly_count: rec.retention_keep_monthly_count,
        retention_auto_delete: rec.retention_auto_delete,
        schedule_kind: rec.schedule_kind,
        schedule_interval_hours: rec.schedule_interval_hours,
        schedule_time: rec.schedule_time,
        schedule_day: rec.schedule_day,
...
pub async fn rec_list_snapshot_files(
    ...
    // Tree-shape diagnostics.  Three separate reports of "this folder will not
    // expand" could not be explained from the UI code, and the tree's parent
    // lookup is pure string work on these paths - so the shape of the path set
    // is the missing evidence.  Counts and separator statistics only; no file
    // names are logged.
    {
        use std::collections::HashSet;
        let all: HashSet<&str> = entries.iter().map(|e| e.path.as_str()).collect();
        let dirs = entries.iter().filter(|e| e.is_dir).count();
        let with_bs = entries.iter().filter(|e| e.path.contains('\\')).count();
        let with_fs = entries.iter().filter(|e| e.path.contains('/')).count();
        let mixed = entries
            .iter()
            .filter(|e| e.path.contains('\\') && e.path.contains('/'))
            .count();
        // Replicate the UI's parentOf(): last '/' OR '\\', with a bare drive
        // letter promoted to root.  An entry whose computed parent is not
        // itself in the set gets promoted to the root level by the tree, so a
        // large `promoted` count means the tree is flattening the hierarchy.
        let parent_of = |p: &str| -> String {
            ...
            match last {
                ...
                Some(i) => {
                    let q = &p[..i];
                    let is_drive = q.len() == 2
                        && q.ends_with(':')
                        && q.chars().next().unwrap().is_alphabetic();
                    if is_drive {
                        String::new()
                    } else {
                        q.to_string()
        ...
        let mut childless_dirs = 0usize;
        let mut have_children: HashSet<String> = HashSet::new();
        for e in &entries {
            let par = parent_of(&e.path);
            if par.is_empty() {
                roots += 1;
            } else if !all.contains(par.as_str()) {
                promoted += 1;
            } else {
                have_children.insert(par);
            }
        }
        for e in entries.iter().filter(|e| e.is_dir) {
            if !have_children.contains(&e.path) {
                childless_dirs += 1;
            }
        }
        tracing::info!(
            total = entries.len(),
            duplicate_paths = entries.len() - all.len(),
            dirs,
            roots,
            promoted,
            childless_dirs,
            with_backslash = with_bs,
            with_forwardslash = with_fs,
            mixed_separators = mixed,
            "rec_list_snapshot_files: tree shape"
        );
    }
...
pub async fn rec_start_restore(
    ...
    let restore_task = tokio::spawn(async move {
        ...
        // Phase tracing.  The 1.0.0 stall showed "pack map built" as the last
        // line in the log and nothing after it, which made it impossible to
        // tell a wedged restore from a slow one.  These two lines bound the
        // engine call so the next report is diagnosable from the log alone.
        tracing::info!(
            files = filter_paths.len(),
            excluded = excluded_paths.len(),
            "rec_start_restore: entering restore_all"
        );
        let result = engine
            .restore_all(
                &snapshot_id,
        ...
        tracing::info!(
            ok = result.is_ok(),
            "rec_start_restore: restore_all returned"
        );
        // Bounded wait.  `progress_handle` only ends when EVERY clone of the
        // progress sender is dropped, and the engine hands clones to a
        // heartbeat task and to each per-file task.  One clone outliving
        // `restore_all` would block this await forever - and the finalise
        // block below, which is the only place a terminal status is written,
        // sits after it.  The UI polls that status to clear `restoring`, so
        // the cost of hanging here is a permanently locked recovery screen
        // with no error and no way out but killing the app.  Time-box it:
        // a late progress event is worth less than a usable UI.
        if tokio::time::timeout(std::time::Duration::from_secs(10), progress_handle)
            .await
            .is_err()
        {
            tracing::warn!("restore: progress task still alive after 10s - finalising anyway");
        }
        ...
        match result {
            Ok(()) => {
                // Unconditional: a None here would otherwise leave the UI
                // polling "running" forever.
                let v = s.progress.get_or_insert_with(terminal_progress_view);
    ...
    // Supervisor.  `tokio::spawn` swallows a panic: the task just disappears,
    // no terminal status is ever written, and the UI polls "running" for the
    // life of the process with the snapshot picker, the file tree and Back
    // all disabled.  In a recovery tool that is the worst possible failure
    // mode - it is reached by someone who has already lost data - so the
    // guarantee here is that the task CANNOT end without a terminal status,
    // whatever killed it.
    let supervisor_session: SharedSession = (*session).clone();
    tokio::spawn(async move {
        let outcome = restore_task.await;
        let mut s = supervisor_session.write().await;
        let unfinished = match s.progress.as_ref() {
            Some(v) => !matches!(v.status.as_str(), "complete" | "error" | "cancelled"),
            None => true,
        };
        if unfinished {
            let detail = if outcome.is_err() {
                "The restore stopped unexpectedly and reported no result. \
                 Nothing was changed at the destination beyond files already \
                 written.  Retry the restore; if it stops again, check the \
                 log for the last file attempted."
            } else {
                "The restore ended without reporting a result.  Retry it; \
                 if it happens again, check the log for the last file \
                 attempted."
            };
            tracing::error!(
                panicked = outcome.is_err(),
                "restore task ended without a terminal status - forcing error"
            );
            let v = s.progress.get_or_insert_with(terminal_progress_view);
            v.status = "error".into();
            v.error_detail = detail.into();
            v.paused = false;
        }
        s.cancel_tx = None;
        s.pause_tx = None;
...
/// A blank progress view to attach a terminal status to when the live one is
/// gone.  The counters are zero because they are unknown at that point; the
/// status and `error_detail` are what the UI needs to leave its locked state.
fn terminal_progress_view() -> RestoreProgressView {
    RestoreProgressView {
        status: "error".into(),
        files_done: 0,
        files_total: 0,
