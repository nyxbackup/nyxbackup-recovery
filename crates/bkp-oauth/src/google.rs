...
// REQ-095: drive.appdata ONLY. The main app writes its encrypted backups to
// Google Drive's hidden per-app appDataFolder; this recovery tool must request
// the SAME scope AND embed the SAME OAuth client id to read them back
// (drive.appdata visibility is per-client-id). This is a SENSITIVE scope
// (standard OAuth verification, NO CASA) - the full `drive` restricted scope was
// dropped to avoid the CASA security assessment. There is no user-chosen folder.
...
pub async fn exchange_code(
    ...
    // drive.appdata: the destination is always the hidden appDataFolder, never a
    // user-chosen folder. `folder_url` is accepted for signature compatibility.
    let _ = folder_url;
    let folder_id = "appDataFolder".to_string();
    ...
    let has_drive = granted
        .split(' ')
        .any(|s| s == "https://www.googleapis.com/auth/drive.appdata");
    if !granted.is_empty() && !has_drive {
        anyhow::bail!(
            "Google did not grant Drive access (granted scopes: [{granted}]). The \
             OAuth app needs the 'drive.appdata' scope approved in the Google Cloud \
