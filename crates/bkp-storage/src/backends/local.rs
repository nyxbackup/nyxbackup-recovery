...
impl StorageBackend for LocalBackend {
    ...
    async fn probe_access(&self) -> Result<()> {
        // `Path::is_dir` is a synchronous stat and an `async fn` body runs on a
        // tokio WORKER thread.  A `local` path is routinely a network mount -
        // that is how this backend is used on Linux and macOS - so against a
        // hung mount this held a worker until the OS gave up.
        let root = self.full_path("");
        let probe = root.clone();
        let is_dir = tokio::task::spawn_blocking(move || probe.is_dir())
            .await
            .map_err(|e| Error::Storage(format!("probe_access join: {e}")))?;
        if is_dir {
            Ok(())
        } else {
    ...
    async fn exists(&self, path: &str) -> Result<bool> {
        // Same reasoning as `probe_access`.
        let full = self.full_path(path);
        tokio::task::spawn_blocking(move || full.exists())
            .await
            .map_err(|e| Error::Storage(format!("exists join: {e}")))
    }
