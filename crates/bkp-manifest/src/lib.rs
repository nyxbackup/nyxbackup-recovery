...
/// A per-backup-set descriptor stored at `sets/<set_id>/set-record`, encrypted
/// with that set's `ManifestEncryption` subkey (REQ-060 companion).
///
/// Self-describing, storage-only recovery/management metadata: it lets the
/// Recovery Tool answer "what *is* this set?" (source paths, retention,
/// schedule, owning machine/user) from the destination + master key alone.
/// Optional and purely descriptive - restore never depends on it; a set that
/// has never run since the feature shipped has none.  Must stay field-for-field
/// identical to the writer in the main app (format parity).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SetRecord {
    /// On-disk format version; see [`CURRENT_FORMAT_VERSION`].
    pub format_version: u32,
    /// Stable identifier of the backup set this record describes.
    pub backup_set_id: BackupSetId,
    /// Machine that owns the set.
    pub machine_id: MachineId,
    /// User-visible set name.
    pub set_name: String,
    /// Hostname of the owning machine at last write.
    pub hostname: String,
    /// OS username that owns the set (informational; may be empty).
    pub username: String,
    /// Whether the set is enabled for scheduled backups.
    pub enabled: bool,
    /// Source paths included in the set.
    pub include_paths: Vec<String>,
    /// Exact-prefix exclude paths.
    pub exclude_paths: Vec<String>,
    /// Filename glob exclude patterns.
    pub exclude_patterns: Vec<String>,
    /// Excluded file extensions (without the leading dot).
    pub exclude_extensions: Vec<String>,
    /// Provider type string of the primary endpoint (e.g. `"s3"`, `"local"`).
    pub endpoint_type: String,
    /// Non-secret human descriptor of the primary endpoint's destination.
    pub endpoint_target: String,
    /// Retention: keep every snapshot newer than this many days.
    pub retention_keep_all_days: u32,
    /// Retention: keep one snapshot per week for this many weeks.
    pub retention_keep_weekly_count: u32,
    /// Retention: keep one snapshot per month for this many months.
    pub retention_keep_monthly_count: u32,
    /// Retention: whether expired snapshots are auto-deleted.
    pub retention_auto_delete: bool,
    /// Human-readable schedule kind: `"manual"`, `"continuous"`, `"hourly"`,
    /// `"every_n_hours"`, `"daily"`, or `"weekly"`.
    pub schedule_kind: String,
    /// Interval in hours for the `every_n_hours` schedule (0 when n/a).
    pub schedule_interval_hours: u32,
    /// Local time `"HH:MM"` for daily/weekly schedules (empty when n/a).
    pub schedule_time: String,
    /// Day of week for weekly schedules (empty when n/a).
    pub schedule_day: String,
    /// Time this record was first written, as Unix seconds (UTC).
    pub created_at: u64,
    /// Time this record was last written, as Unix seconds (UTC).
    pub updated_at: u64,
}

/// Decrypt and deserialise a set-record envelope.
///
/// Refuses any `format_version` other than [`CURRENT_FORMAT_VERSION`], matching
/// the other decoders in this crate.
pub fn decode_set_record(data: &[u8], key: &SubKey) -> Result<SetRecord> {
    let plaintext = envelope::decode(key, data)?;
    let record: SetRecord = cbor_decode(&plaintext)?;
    if record.format_version != CURRENT_FORMAT_VERSION {
        return Err(bkp_types::error::Error::Serialization(format!(
            "set-record format_version {} not supported by this build (expected {})",
...
/// Remote path for a backup set's encrypted set-record ([`SetRecord`]).
///
/// Pattern: `sets/<backup_set_id>/set-record`
pub fn set_record_remote_path(backup_set_id: &BackupSetId) -> String {
    format!("sets/{}/set-record", backup_set_id.as_uuid())
}
...
#[cfg(test)]
mod bootstrap_record_compat_tests {
    use super::decode_bootstrap_record;
    use bkp_types::backup_set::BackupSetId;
    use bkp_types::machine::{Argon2Params, BootstrapRecord, MachineId};

    /// Encode with ciborium directly: this crate exposes only a decoder,
    /// because the Recovery Tool reads backups and never writes them.
    ...
    fn record(kdf: Option<Argon2Params>) -> BootstrapRecord {
        BootstrapRecord {
            format_version: 2,
            machine_id: MachineId::from_uuid(uuid::Uuid::nil()),
            hostname: "some-machine".to_string(),
            created_at: 1_757_200_000,
            kdf_params: kdf,
            backup_set_ids: vec![BackupSetId::from_uuid(uuid::Uuid::nil())],
        }
    }

    /// The case this tool used to refuse outright.
    ///
    /// `kdf_params` was a required field, so a bootstrap record written
    /// without it failed to decode - and current builds of Nyx Backup never
    /// write it, because the master key is random rather than derived from a
    /// passphrase.  That made the common case undecodable: the tool could not
    /// read the record telling it which machine and which backup sets a
    /// destination holds.
    #[test]
    fn decodes_a_record_with_no_kdf_params() {
        let back = decode_bootstrap_record(&encode(&record(None))).expect("decode");
        assert!(back.kdf_params.is_none());
        assert_eq!(back.hostname, "some-machine");
        assert_eq!(back.backup_set_ids.len(), 1);
    }

    /// Records from older machines still carry the field and must keep
    /// working - that is the population that can still re-derive a key from a
    /// passphrase, so losing them would remove a recovery path.
    #[test]
    fn decodes_a_record_that_has_kdf_params() {
        let bytes = encode(&record(Some(Argon2Params::default())));
        let back = decode_bootstrap_record(&bytes).expect("decode");
        let params = back.kdf_params.expect("params must survive the round trip");
        assert_eq!(params.salt.len(), 32);
    }

    /// A payload that omits the key entirely - what an older writer emits -
    /// must decode rather than error.  This is what `serde(default)` is for,
    /// and it is the difference between reading someone's backup and telling
    /// them it is corrupt.
    #[test]
    fn decodes_a_payload_missing_the_field_entirely() {
        ...
        struct Older {
            ...
            hostname: String,
            created_at: u64,
            backup_set_ids: Vec<BackupSetId>,
        }
        let older = Older {
            format_version: 2,
            machine_id: MachineId::from_uuid(uuid::Uuid::nil()),
            hostname: "older-writer".to_string(),
            created_at: 1,
            backup_set_ids: Vec::new(),
        };
        let back = decode_bootstrap_record(&encode(&older)).expect("decode older shape");
        assert!(back.kdf_params.is_none());
        assert_eq!(back.hostname, "older-writer");
    }
}
