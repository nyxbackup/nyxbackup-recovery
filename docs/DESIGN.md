...
A recovery tool is used when the original machine is gone, so it must not
require the user to prepare the system first.  Every backend therefore speaks
its own protocol in-process using credentials the user types, including SMB:
on Linux the SMB backend opens an SMB2/3 session through libsmbclient rather
than requiring a mounted share.  That is not a convenience - an unprivileged
GUI cannot mount CIFS at all (the kernel wants `CAP_SYS_ADMIN`, and setuid
`mount.cifs` only serves mount points already in `/etc/fstab`), so a
mount-based backend would have made SMB recovery impossible off a live USB.

An existing mount is still used when `mount_path` points at one.  macOS keeps
the mount requirement (no packaged libsmbclient) and says so with a
ready-to-run mount command.  See `docs/LINUX_COMPAT.md` for the packaging and
threading consequences.
...
- **Installers** live on `downloads.nyxbackup.com`.  The web hub at
  `nyxbackup.com/download-recovery` - the one URL the app's About screen opens - links to
