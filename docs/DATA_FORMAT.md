...
- **`[u8; N]` is Rust array notation** meaning "a fixed-length
  ...
  holding a 128-bit UUID (~3.4×10³⁸ possible values), not an 8-bit
...
```
BootstrapRecord {
    format_version: u32,                  // = 2 (CURRENT_FORMAT_VERSION); not enforced on decode
...
```
SnapshotIndex {
    format_version: u32,                  // = 2 (REQ-047.4 stable version)
...
SnapshotEntry {
    ...
    created_at: u64,                      // Unix SECONDS (not nanoseconds)
...
SetRecord {
    ...
    set_name: text,
    hostname: text,
    username: text,                       // OS user; "" if undeterminable
    enabled: bool,
    include_paths: [text, ...],
    exclude_paths: [text, ...],
    exclude_patterns: [text, ...],
    exclude_extensions: [text, ...],
    endpoint_type: text,                  // "s3", "local", ...
    endpoint_target: text,                // non-secret descriptor, e.g. "s3://bucket/prefix"
    retention_keep_all_days: u32,
    retention_keep_weekly_count: u32,
    retention_keep_monthly_count: u32,
    retention_auto_delete: bool,
    schedule_kind: text,                  // "manual"|"continuous"|"hourly"|"every_n_hours"|"daily"|"weekly"
    schedule_interval_hours: u32,         // 0 when n/a
    schedule_time: text,                  // "HH:MM" or ""
    schedule_day: text,                   // weekday name or ""
    created_at: u64,                      // Unix seconds; first write, preserved across rewrites
    updated_at: u64,                      // Unix seconds; last descriptive change
}
```
...
manifest, and snapshot index) are published as a downloadable
tarball alongside this spec at
<https://nyxbackup.com/format/test-vectors-vX.Y.Z.tar.gz>.
...
*Document version aligned with Nyx Backup v0.9.24.  Format
version 2.  Stable.*
