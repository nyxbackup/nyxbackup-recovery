...
<script lang="ts">
  ...
  /**
   * Paint a checkbox's two DOM properties.
   *
   * `indeterminate` has no HTML attribute, so a partially selected folder
   * rendered as an EMPTY box - indistinguishable from "nothing selected".
   *
   * Taking the values as PARAMETERS is what makes this reactive: `{@attach
   * expr}` tracks only what the expression itself reads, so an attachment
   * written inline as `(el) => { el.checked = ... }` reads its state inside
   * the returned function where Svelte cannot see it, runs once at mount, and
   * then paints a stale box for the life of the row.
   */
  function paintCheck(checked: boolean, indeterminate: boolean) {
  ...
  // ── Duplicate paths ──────────────────────────────────────────────────────────
  //
  // The row `{#each}` below is KEYED on `entry.path`, and Svelte throws on a
  // duplicate key - which aborts the render and leaves the whole restore pane
  // painted but inert, taking no clicks at all.  A snapshot is not supposed to
  // list a path twice, but manifests written by 0.9.47-0.9.49 on Windows do:
  // the engine's empty-directory marker injection compared raw backslash paths
  // with a forward-slash ancestor walk, so it emitted a marker for directories
  // that already held files.  Those manifests live in remote storage and cannot
  // be rewritten, so browsing one has to keep working.  First entry wins; the
  // duplicate carries the same path and, for a directory, nothing else the tree
  // renders.
  //
  // Ported from the main application, where this was already fixed.  The
  // recovery fork was lifted before that fix landed, so it inherited the bug
  ...
  let childrenMap = $derived((() => {
    ...
    const entryPaths = new Set(uniqueFiles.map(f => f.path))
  ...
  const virtualWindow = $derived((() => {
    ...
    // Clamped.  `viewportHeight` comes from clientHeight, and a flex child
    // whose min-height resolves to auto does not shrink below its content -
    // so the scroll container grew to fit the whole tree and clientHeight
    // reported the CONTENT height, not the viewport.  visibleCount then came
    // out as "every row", the windowing did nothing, and expanding a folder
    // with a few thousand entries rendered all of them at once and hung the
    // screen.  min-h-0 on the flex chain is the actual fix; this clamp means
    // a future layout change cannot quietly reintroduce an unbounded render.
    const MAX_ROWS = 300
    const visibleCount = Math.min(MAX_ROWS, Math.ceil(viewportHeight / ROW_HEIGHT))
  ...
  // ── Selection helpers ─────────────────────────────────────────────────────────
  /**
   * Descendants of a directory, including itself.
   *
   * Only consulted for a directory that carries no rule of its own, which is
   * the uncommon case; `stateFor` takes it as a callback for that reason.
   */
  ...
  function entryState(entry: SnapshotFile): CheckState {
    return stateFor(entry.path, entry.is_dir, selection, excludedPaths, descendantPaths)
  }

  /** True when the nearest rule covering `path` is an exclusion. */
  function entryExcluded(path: string): boolean {
    return ruleFor(path, selection, excludedPaths)?.include === false
  }

  function toggle(entry: SnapshotFile) {
    const next = toggleRule(entry.path, selection, excludedPaths)
...
>
  ...
  <!-- Tree (windowed renderer; only viewport rows live in the DOM).
       role="listbox" belongs HERE, not on the outer wrapper: it must contain
       only the role="option" rows, and the wrapper also holds the header and
       the footer hint.  tabindex="0" is what actually makes the keyboard nav
       reachable - the handler sits on the wrapper and bubbles here, but with
       nothing tabbable a keyboard user could never focus the tree to use it.
       Focus stays on this container and aria-activedescendant names the active
       row (roving-focus model), so the virtualised rows need no real focus. -->
  <div
    ...
    role="listbox"
    tabindex="0"
    aria-label={t('gui.filetree.aria_label')}
    aria-activedescendant={visibleNodes[cursor] ? `nyx-ft-row-${cursor}` : undefined}
  >
    ...
    {:else}
      ...
      {#each visibleNodes.slice(virtualWindow.start, virtualWindow.end) as node, sliceIdx (node.entry.path)}
        ...
        {@const isExcluded = entryExcluded(entry.path)}
        <!-- Rows under an excluded folder stay CLICKABLE: `toggle` pushes the
             exclusion down a level so a single file can be pulled back out of
             a deselected folder.  Disabling them instead made that selection
             unexpressible. -->
        ...
        >
          ...
          {#if entry.is_dir}
            <button
              ...
              title={!node.hasChildren
                       ? t('gui.filetree.empty_dir')
                       : isExpanded ? t('gui.action.collapse') : t('gui.action.expand')}
            >{node.hasChildren ? (isExpanded ? '▼︎' : '▶︎') : '·'}</button>
          {:else}
          ...
          <input
            type="checkbox"
            {@attach paintCheck(state === 'full', state === 'partial')}
          ...
          <!-- Icon + name -->
          <FileIcon kind={entry.is_dir ? 'dir' : entry.is_symlink ? 'symlink' : 'file'} />
  ...
  {#if contextMenu}
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <!-- tabindex="-1": a menu is focused programmatically, never tabbed to.
         Escape closes it via the wrapper's keydown handler. -->
    <div
      role="menu"
      tabindex="-1"
      aria-label={t('gui.filetree.menu_aria_label')}
