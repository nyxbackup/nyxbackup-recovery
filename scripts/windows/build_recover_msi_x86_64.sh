#!/usr/bin/env bash
# Copyright (c) 2026 Nyx Software, LLC
# SPDX-License-Identifier: Apache-2.0
# Nyx Backup Recovery - https://nyxbackup.com
# Build the standalone Nyx Backup Recovery MSI.
#
# Assumes scripts/windows/build_windows_x86_64.sh has already run so that
# staging/windows/x86_64/nyx_bkp_recover.exe + WebView2Loader.dll exist.
# Pass --build to chain a fresh build first.
#
# Output: dist/NyxBackup-Recovery-VERSION-x86_64.msi  (~33 MB - much
# smaller than the main MSI; the recovery binary plus WebView2Loader is
# all it ships).

set -euo pipefail

WORKSPACE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

# Load .env so the MSI-level Authenticode sign gate sees NYX_TS_*/AZURE_* when
# this script is run standalone.  build_windows_*.sh sources it for the per-exe
# signing; the MSI sign step runs in THIS script's env, so it needs its own load
# or the recovery MSI ships unsigned ("Unknown publisher" on UAC).
if [[ -f "${WORKSPACE_DIR}/.env" ]]; then
    set -a; source "${WORKSPACE_DIR}/.env"; set +a
fi

STAGING="${WORKSPACE_DIR}/staging/windows/x86_64"
WXS="${WORKSPACE_DIR}/installer/windows/NyxBackupRecovery-x86_64.wxs"
DIST="${WORKSPACE_DIR}/dist"
DO_BUILD=0

for arg in "$@"; do
    case "$arg" in
        --build) DO_BUILD=1 ;;
        *) echo "Unknown argument: $arg"; exit 1 ;;
    esac
done

VERSION=$(cat "$WORKSPACE_DIR/VERSION" | tr -d '[:space:]')
echo "Building Recovery MSI for Nyx Backup v${VERSION}..."

command -v wixl >/dev/null 2>&1 || {
    echo "ERROR: wixl not found.  Install msitools: sudo apt install msitools"
    exit 1
}

if [[ $DO_BUILD -eq 1 ]]; then
    bash "${WORKSPACE_DIR}/scripts/windows/build_windows_x86_64.sh"
fi

for f in nyx_bkp_recover.exe WebView2Loader.dll; do
    [[ -f "$STAGING/$f" ]] || {
        echo "ERROR: missing $STAGING/$f"
        echo "  Run with --build, or run scripts/windows/build_windows_x86_64.sh first."
        exit 1
    }
done

# Same wixl-vs-DOS-timezone dance the main MSI does.  See build_msi_x86_64.sh
# for the rationale comment.  The payload mtimes are set to Windows LOCAL time
# but stamped under TZ=UTC, so the DOS timestamps wixl writes come out reading
# as local time on the target machine.
#
# Getting that local time from powershell.exe is best-effort, not a
# requirement.  Under WSL launched from a Windows PowerShell session, the
# parent's PATH leaks in mangled and the nested powershell.exe fails - which,
# with `set -euo pipefail`, used to abort the whole script right after staging
# and before wixl ever ran, reporting success.  So: probe, validate the shape
# of what came back, and fall back to WSL's own local clock (which follows the
# Windows timezone) when it is unusable.
WIN_TIME="$(powershell.exe -NoProfile -Command "Get-Date -Format 'yyyyMMddHHmm'" 2>/dev/null | tr -d '\r\n' || true)"
if [[ ! "$WIN_TIME" =~ ^[0-9]{12}$ ]]; then
    WIN_TIME="$(date +%Y%m%d%H%M)"
    echo "powershell.exe unavailable - using local time ${WIN_TIME} for payload timestamps."
fi
( export TZ=UTC
  touch -t "$WIN_TIME" "$STAGING/nyx_bkp_recover.exe" "$STAGING/WebView2Loader.dll" )

mkdir -p "$DIST"
OUTPUT="${DIST}/NyxBackup-Recovery-${VERSION}-x86_64.msi"

wixl \
    --arch x64 \
    --define "Version=${VERSION}" \
    --define "StagingDir=${STAGING}" \
    --output "$OUTPUT" \
    "$WXS"

if [[ ( -n "${NYX_SIGN_CERT:-}" && -n "${NYX_SIGN_KEY:-}" ) || -n "${NYX_TS_PROFILE:-}" ]]; then
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    "${SCRIPT_DIR}/sign_pe.sh" "$OUTPUT"
fi

SIZE=$(du -h "$OUTPUT" | cut -f1)
echo ""
echo "Recovery MSI created: $OUTPUT  ($SIZE)"
echo ""
echo "Install:  msiexec /i NyxBackup-Recovery-${VERSION}-x86_64.msi"
