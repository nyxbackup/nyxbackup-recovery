...
A standalone, open-source recovery tool for
...
The complete on-disk archive format (envelope, chunking, pack layout,
manifest schema, key derivation) is published as a specification with
reference test vectors at <https://nyxbackup.com/format>, so an
independent reader can be built from the spec alone - this tool is one
such reader.
...
- Read-only: it never writes to, deletes from, or modifies your backup
  storage.  It only downloads.  For OAuth providers this is enforced by the
  provider, not just by convention: OneDrive is requested with `Files.Read`
  and Dropbox with a read-only scope subset, so the token this tool obtains
  cannot write to or delete your cloud storage. (Google Drive uses the
  app-private `drive.appdata` scope, which has no read-only variant but only
  ever exposes this app's own hidden folder.)
...
- **Installers:** the download hub at <https://nyxbackup.com/download-recovery> links to
  the signed installers on `downloads.nyxbackup.com`.
- **Verify what you download:** every release from 1.0.0 onward ships
  `SHA256SUMS-<ver>.txt` and a detached GPG signature
  ...
  project controls.  (Some pre-1.0 releases were published without a signature;
  the tooling that allowed that has been fixed.)
- **Verify the source, not just the binaries:** release tags are GPG-signed
  with the same key, so `git tag -v v1.0.0` confirms that the tree you are
  reading is the tree the release was cut from.  Checking only the installer
  tells you Nyx Software built *something*; checking the tag tells you it was
  built from *this*.

## Installing

Grab the installer for your OS and CPU, **verify the checksum and the GPG
signature**, then install.  Full instructions (all platforms,
uninstall, WSL notes) are in [`docs/INSTALL.md`](docs/INSTALL.md).

```bash
# Verify first (run where the installer + sums file live)
gpg --verify SHA256SUMS-<ver>.txt.asc SHA256SUMS-<ver>.txt   # authenticity
sha256sum -c SHA256SUMS-<ver>.txt                            # integrity

# And, if you cloned the source, that this tree is the released one:
git tag -v v<ver>
