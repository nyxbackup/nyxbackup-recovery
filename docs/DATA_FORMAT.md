...
Every format version and what changed in it, per REQ-066.4.  A change to the
envelope, the CBOR schemas, the chunk-ID derivation, or any signed-input layout
bumps this version; a purely additive, ignorable field does not.

| Version | Nyx Backup | Change |
|---------|-----------|--------|
| **2** | 0.5.108 and later | **Current.**  Chunk IDs are HMAC-SHA256 over the plaintext under a per-set subkey, and integrity throughout uses the SHA-256 family.  The same plaintext therefore has a *different* content address than under v1, so a v2 reader cannot interpret a v1 archive and vice versa.  This was a pre-release break: the migration was to reset every backup set. |
| **1** | before 0.5.108 | Initial release.  Used a pre-FIPS content-hash primitive for chunk IDs and the manifest authenticator.  **No longer accepted** by any current reader. |

Notes for implementers:

- The version lives in `format_version` on every manifest, snapshot index and
  machine record; it is not carried per pack or per chunk.  Read it before
  interpreting anything else in the object.
- The jump from 1 to 2 is the only break so far, and it happened before the
  product was generally available.  A future break would require a documented
  migration path over months rather than a reset, and, per section 10, a v3
  reader must still read v2 archives.
- If this table and `bkp_manifest::CURRENT_FORMAT_VERSION` ever disagree, the
  code is authoritative for what a build accepts, and the disagreement is a bug
  in this document.

## 12. Verifying an implementation
...
## 14. Open-source reference reader
