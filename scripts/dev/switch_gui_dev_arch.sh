...
# NOT in the list above, deliberately: libsmbclient-dev.
#
# The SMB backend links libsmbclient, so a cross-build does need the :arm64
# headers - but installing them through apt is destructive on an amd64 host.
# libsmbclient-dev pulls Samba's python3 bindings, so apt resolves
# `libsmbclient-dev:arm64` by REMOVING the host's amd64 python3 and unpacking
# arm64 python3 in its place, which then cannot execute:
#
#   /var/lib/dpkg/info/python3.10-minimal.postinst: /usr/bin/python3.10: Exec format error
#
# That leaves the build host without python3 and mid-transaction.  The arm64
# headers are therefore unpacked into a sysroot instead of installed - see
# fetch_arm64_smbclient() in scripts/linux/build_linux_arm64.sh.
