...
<script lang="ts">
  ...
  async function connectOnedrive() {
    ...
    try {
      ...
      connectForm.storageSecret = r.secret
      fieldChanged()
      connMessage = tf('gui.recover.connect.connected_as', {
                  account: r.email || t('gui.recover.connect.unknown_account'),
                })
    } catch (e) {
      errorMsg = friendlyError(e)
    } finally {
    ...
    try {
      if (connectForm.storageType === 'google_drive') {
        const r = await invoke<{ secret: string; folder_id: string; email: string }>(
          ...
          { folderUrl: 'appDataFolder', pasted: manualPasted, redirectUri: manualRedirectUri })
...
<div class="max-w-3xl mx-auto p-6 flex flex-col gap-5">
  ...
  <section class="rounded-lg border border-nyx-border bg-nyx-surface p-5 flex flex-col gap-3">
    ...
    <!-- Google Drive is appdata-only (hidden private folder), so there is no
         user-chosen path - just the Connect button below. Every other backend
         keeps the URL/path field. -->
    {#if connectForm.storageType !== 'google_drive'}
    <div class="flex flex-col gap-1.5">
    ...
    {#if connectForm.storageType === 'smb'}
      <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-col gap-1">
          ...
          />
          <p class="text-[10px] text-nyx-muted">
            {@html t('gui.recover.connect.mount_hint')}
          </p>
        </div>
      </div>
    {/if}
    ...
    {#if connectForm.storageType === 'google_drive' || connectForm.storageType === 'onedrive' || connectForm.storageType === 'dropbox'}
      <div class="flex flex-col gap-2">
        ...
        {#if connectForm.storageType === 'google_drive'}
            ...
            onclick={async () => {
              ...
              try {
                const r = await invoke<{ folder_id: string; refresh_token: string; email: string }>(
                  'rec_google_oauth', { folderUrl: 'appDataFolder' })
                connectForm.storageUrl = r.folder_id
                connectForm.storageSecret = r.refresh_token
                fieldChanged()
                connMessage = tf('gui.recover.connect.connected_as', {
          ...
          <!-- Drive is appdata-only: one flat per-account folder with no
               user-chosen path, so EVERY set backed up to this account shows
               up - including sets from other machines.  Say so here rather
               than letting the multi-set list surprise the user later. -->
          <p class="text-[11px] text-nyx-muted leading-snug">
            {t('gui.recover.connect.gdrive_appdata_note')}
          </p>
        {/if}
    ...
    <div class="flex items-center gap-3 mt-1">
      ...
      {:else if connTested === 'fail'}
        <span class="text-xs text-nyx-error whitespace-pre-wrap break-words">✗ {connMessage}</span>
      {:else}
