...
# Guard: a failed `cd` above leaves WORKSPACE_DIR empty (masked by $()), which
# would make PKG_ROOT an absolute "/target/..." path - so `rm -rf "$PKG_ROOT"`
# and the install steps would hit the wrong location (or create a stray
# root-level tree).  Fail loudly instead.
[[ -n "$WORKSPACE_DIR" && -d "$WORKSPACE_DIR" ]] || {
    echo "ERROR: could not resolve WORKSPACE_DIR from the script path." >&2
    exit 1
}
# Load .env so the signing gate below sees NYX_SIGN_KEY_ID.  build_linux_*.sh
# sources .env too, but that runs as a subprocess - its exports do not reach
# back here, and the debsigs step runs in THIS script's env.  Without its own
# load the gate silently takes the "unsigned (dev build)" branch and still
# exits 0, so a release build looks clean while shipping an unsigned .deb.
...
# -Zgzip: force gzip members (control.tar.gz / data.tar.gz).  Modern dpkg-deb
# defaults to zstd, which debsigs 0.1.x cannot parse (it signs a bad
# concatenation -> BAD signature).  gzip matches the main Nyx Backup .deb and is
# universally supported by apt/dpkg.
dpkg-deb --build -Zgzip --root-owner-group "$PKG_ROOT" "$OUTPUT"
rm -rf "$PKG_ROOT"
...
# ── Optional GPG signing (debsigs) ────────────────────────────────────────────
# Signs with the SAME release key as the main Nyx Backup packages
# (NYX_SIGN_KEY_ID). Unset -> unsigned dev build.
if [[ -n "${NYX_SIGN_KEY_ID:-}" ]]; then
    echo "NYX_SIGN_KEY_ID set - signing .deb with debsigs..."
    bash "${WORKSPACE_DIR}/scripts/linux/sign_deb.sh" "$OUTPUT" "$NYX_SIGN_KEY_ID"
    echo ""
else
    echo "NYX_SIGN_KEY_ID not set - .deb is unsigned (dev build)."
    echo ""
fi
