...
/// Reply for `rec_list_snapshots`: the snapshots, plus a count of backup sets
/// found at the endpoint that could NOT be decrypted with the in-session
/// master key.
///
/// The count matters for a panicking user.  Master keys are per-machine, and
/// some endpoints - notably Google Drive, whose `appDataFolder` is one flat
/// per-account space - hold sets from several machines side by side.  Dropping
/// the ones that fail to decrypt makes a present backup look like a missing
/// one, which is the worst possible impression during a disaster recovery.
/// Reporting the count lets the GUI say "try that machine's key" instead.
#[derive(Debug, Clone, serde::Serialize)]
pub struct SnapshotListReply {
    pub snapshots: Vec<SnapshotSummary>,
    /// Sets whose snapshot-index failed to decode (wrong master key, or the
    /// legacy cipher).  Zero in the normal single-machine case.
    pub undecryptable_sets: u32,
}
